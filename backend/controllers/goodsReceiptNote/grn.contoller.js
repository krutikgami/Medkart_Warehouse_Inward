...
const addGrn = async (req, res) => {
  try {
    ...
    const exist_grnCode = await prisma.grn.findFirst({
      where:{
        grn_code : grnCode
      }
    })
    ...
    if(exist_grnCode){
      return res.status(400).json({
        success : false,
        message : "Grn Code already existed" 
      })
    }
    ...
    const purchaseOrderQuantity = existingPO.items.reduce(
      (sum, item) => sum + Number(item.quantity),
      0
    )
    const newGrnItemsQuantity = newGrn.items.reduce(
      (sum, item) => sum + Number(item.quantity),
      0
    )
...
const editGrn = async (req, res) => {
  try {
    ...
    const newGrnItemsQuantity = updatedGrn.items.reduce(
      (sum, item) => sum + Number(item.quantity),
      0
    )
    const purchaseOrderQuantity = existingPO.items.reduce(
